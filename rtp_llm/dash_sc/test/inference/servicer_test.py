...
class IterRealModelStreamInferTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_logprobs_stay_aligned_across_term_echo_eos_and_phase2_slices(
        ...
        # Prompt echo and injected close delimiters are honest synthetic frames:
        # no model probability is fabricated for either.
        for synthetic_idx in (0, 2):
            self.assertIsNone(_fp32_output(chunks[synthetic_idx], "token_logprobs"))
            self.assertIsNone(
                _int32_output(chunks[synthetic_idx], "top_logprob_token_ids")
            )
            self.assertIsNone(_fp32_output(chunks[synthetic_idx], "top_logprobs"))
