...
class MlaFlashInferDecodeOp(object):
    ...
    def _build_fp8_prefill_absorb_plan(
        ...
        if self.use_cuda_graph:
            # CUDA graph mode: use (B, max_q_len, topk) layout so that
            # plan_prefill_cuda_graph_replay can update indices device-only.
            indices = torch.full(
            ...
            # Eager mode: use (total_q, 1, topk) layout (glm5_cu13 optimisation).
            indices = torch.full(
            ...
            topk_length = torch.ones(
                total_q, dtype=torch.int32, device=block_table.device
            )
            for i in range(B):
                q_len = int(q_lens_h[i].item())
                kv_len = int(kvlen_h[i].item())
