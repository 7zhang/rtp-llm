...
constexpr size_t  kRecordMaxBytes     = 64ULL * 1024ULL * 1024ULL;
constexpr size_t  kQueueMaxBytes      = 256ULL * 1024ULL * 1024ULL;
constexpr int64_t kEventWaitTimeoutUs = 30 * 1000 * 1000;
constexpr size_t  kPayloadOverhead    = 16 * 1024;
...
#define MODEL_INPUT_TENSORS(X)                                                                                       \
    ...
    X(kv_cache_block_id) X(kv_cache_kernel_block_id) X(kv_cache_layer_to_group) X(kv_cache_group_types)             \
    X(kv_cache_update_mapping) X(text_tokens_mask) X(mm_features_locs) X(input_embeddings_locs)                     \
...
bool addBytes(size_t& total, size_t bytes) {
    if (total > kRecordMaxBytes || bytes > kRecordMaxBytes - total) {
        total = kRecordMaxBytes + 1;
        return false;
    }
    total += bytes;
    return true;
...
void addTensorBytes(size_t& total, const torch::Tensor& tensor) {
    if (!tensor.defined() || total > kRecordMaxBytes) {
        return;
    }
    const auto element_size = tensor.element_size();
    const auto numel        = static_cast<size_t>(tensor.numel());
    addBytes(total,
             element_size && numel > kRecordMaxBytes / element_size ? kRecordMaxBytes + 1 : numel * element_size);
}
...
size_t estimateBytes(const GptModelInputs& inputs) {
    size_t bytes = kPayloadOverhead;
    for (const auto& trace_id : inputs.trace_ids) {
        if (!addBytes(bytes, sizeof(std::string) + sizeof(uint64_t)) || !addBytes(bytes, trace_id.size())) {
            return bytes;
        }
    }
...
torch::Tensor snapshotTensor(const torch::Tensor& tensor, std::vector<c10::Device>& devices) {
    ...
    return detached.is_contiguous() ? detached.clone() : detached.contiguous();
}
...
void addTensorList(c10::impl::GenericDict&                          payload,
    ...
    for (size_t i = 0; i < tensors->size(); ++i) {
        if ((*tensors)[i].defined() && (*tensors)[i].scalar_type() == torch::kFloat8_e4m3fn) {
            float8_fields.push_back(std::string(name) + "[" + std::to_string(i) + "]");
        }
        values.push_back(snapshotTensor((*tensors)[i], devices));
    }
...
c10::impl::GenericDict snapshotPayload(const GptModelInputs&     inputs,
    ...
    std::vector<std::string> float8_fields;
#define ADD_TENSOR(field) addTensor(payload, #field, inputs.field, devices, float8_fields);
    MODEL_INPUT_TENSORS(ADD_TENSOR)
#undef ADD_TENSOR
    addTensorList(payload, "multimodal_features", inputs.multimodal_features, devices, float8_fields);
    addTensorList(payload, "mm_extra_input", inputs.mm_extra_input, devices, float8_fields);
    addTensorList(payload, "input_embeddings", inputs.input_embeddings, devices, float8_fields);
    payload.insert("float8_e4m3fn_fields", float8_fields);
...
torch::Tensor tensorForDump(const torch::Tensor& tensor) {
    ...
    if (output.scalar_type() == torch::kFloat8_e4m3fn) {
...
public:
    DumpWriter(int64_t rank_id, int backup_count): backup_count_(std::max(backup_count, 1)) {
        ...
        prefix_              = "model_inputs_r" + std::to_string(rank_id) + "_s" + std::to_string(server_id) + "_";
    ...
    bool write(const c10::impl::GenericDict& payload) {
        const auto bytes = torch::pickle_save(c10::IValue(payload));
        const auto now =
            std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch())
                .count();
        std::ostringstream id;
        id << std::setw(20) << std::setfill('0') << now << "_" << std::setw(6) << file_sequence_++;
...
private:
    ...
    void cleanup() {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(output_dir_, ec)) {
            ...
            } else if (name.rfind("." + prefix_, 0) == 0 && name.find(".tmp.") != std::string::npos) {
...
public:
    ...
    void enqueue(const GptModelInputs& inputs, ModelInputsModelRole role, int64_t model_id) {
        ...
        if (disabled_ || bytes > kRecordMaxBytes || !reserve(bytes)) {
...
private:
    ...
    void process(PendingRecord& record) {
        try {
            ...
            if (!writer_.write(record.payload)) {
                disabled_ = true;
            }
        } catch (const std::exception& e) {
            disabled_ = true;
    ...
    bool waitForEvents(const std::vector<std::shared_ptr<c10::Event>>& events) {
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::microseconds(kEventWaitTimeoutUs);
        ...
        try {
            if (!disabled_) {
                ...
                if (dropped > 0) {
                    ...
                    disabled_ = !writer_.write(payload);
                }
            }
        } catch (const std::exception& e) {
            disabled_ = true;
...
ModelInputsLogger::ModelInputsLogger(int64_t rank_id, int backup_count):
    worker_(std::make_unique<Worker>(rank_id, backup_count)) {}
