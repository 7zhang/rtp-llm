...
void GenerateStream::specUpdate(const StreamSpecUpdateInfo& update_info) {
    ...
    *is_context_stream_ = false;
    ...
    if (num_new_tokens == 0) {
        // CompleteTokenIds can reject the whole speculative window when the
        // stream has reached its logical/physical token cap. Still run the
        // normal finish path, but do not touch the speculative token buffers:
        // there is no committed "last token" in this update.
        auto token_logprobs =
            update_info.token_logprobs.defined() ? update_info.token_logprobs.narrow(1, 0, 0) : torch::Tensor();
        auto top_logprob_token_ids = update_info.top_logprob_token_ids.defined() ?
                                         update_info.top_logprob_token_ids.narrow(1, 0, 0) :
                                         torch::Tensor();
        auto top_logprobs =
            update_info.top_logprobs.defined() ? update_info.top_logprobs.narrow(1, 0, 0) : torch::Tensor();
