...
void NormalGenerateStream::updateOutput(const StreamUpdateInfo& update_info) {
    ...
    // PD-sep first-token forward-to-decode branch: only the prefill role should
    // trigger NeedRemoteGenerate + GenerateDone. On the decode side queryPdSep()
    // is also true (the request carries pd_separation=true end-to-end) and
    // update_remote_generate defaults to true on every Executor-built
    // StreamUpdateInfo, so without the role gate this branch fires on the
    // decode-side first sample — closing the stream after a single token before
    // the second decode sample is even queued. Gate on role_type != DECODE so
    // PDFUSION/PREFILL keep the original semantics while DECODE skips it.
    if (!finished_ && queryPdSep() && update_info.update_remote_generate
        && resourceContext().role_type != RoleType::DECODE) {
        RTP_LLM_LOG_DEBUG(
            "[pd_trace] pd_sep prefill forward-to-decode stream=%ld seq_len=%d num_new_tokens=%d",
            streamId(),
            seqLength(),
    ...
    RTP_LLM_LOG_DEBUG(
        "[pd_trace] enqueueGenerateOutput stream=%ld pd_sep=%d role=%d seq_len=%d "
