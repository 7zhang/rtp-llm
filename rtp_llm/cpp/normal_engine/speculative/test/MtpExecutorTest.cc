...
TEST_F(MtpExecutorTest, testComputeMtpTargetLogprobsKeepsOnlyCompactFp32Statistics) {
    ...
    EXPECT_EQ(result.row_logsumexp.scalar_type(), torch::kFloat32);
    EXPECT_EQ(result.row_logsumexp.numel(), logits.size(0));
...
TEST_F(MtpExecutorTest, testComputeMtpTargetLogprobsSelectsOnlyRequestedRows) {
    ...
    EXPECT_EQ(result.row_logsumexp.numel(), 2);
...
TEST_F(MtpExecutorTest, testCudaMtpRowLogSumExpMatchesFp32Reference) {
